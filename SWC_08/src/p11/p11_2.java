...
public class p11_2 {
	...
	public static void main(String[] args) throws IOException {
		...
		BufferedReader k44_r = new BufferedReader(new FileReader(k44_f)); // 인스턴스 f파일을 읽는 buffer인스턴스 br을 생성했다.

		File k44_f1 = new File("C:\\Users\\501-01\\Desktop\\day_data\\20120117.csv"); // File 클래스의 인스턴스 f를 만들어 .dat파일을 불러온다.
		...
		while ((k44_readtxt = k44_r.readLine()) != null) {	//우선적으로 해당 파일의 다음 라인을 readtxt변수에다가 저장을 한다! 그리고 다음라인의 값이 null일 때까지 반복문을 돌린다는 뜻.
			...
			if (k44_field.length > 1 && k44_field[1].replace("^", "").trim().equals("20120117")) {	//항목 갯수가 1개가 넘으면서 동시에 해당 리스트를 정제한 값이 해당 문자열과 같을 때
				k44_wCnt ++; //쓰기 카운트 1추가
