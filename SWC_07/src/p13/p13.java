...
public class p13 {
	...
	public static void main(String[] args) {
		...
		String [] OneRec= {	//배열 안에 내용이 또 ","로 나뉘어 질 수 있으므로 이 배열은 2차원 배열로 볼 수 있다.
		...
		String[] field_name = OneRec[0].split(",");	//field_name은 OneRec 배열의 첫번째 요소(OneRec[0])를 ","으로 나눈 것이고 그것 또한 배열. 즉 OneRec은 2차 배열느낌.
		
		for(int i=1; i<OneRec.length-1;i++) {	//항목(0)번째를 제외한 나머지 배열안 배열들을 반복문으로 훑는다.
			
			String[] field = OneRec[i].split(",");	//훑는 해당 라인을 ","을 기준으로 field배열에 담는다.
			System.out.println("*******************************");
			for(int j=0; j<field_name.length; j++) {	//항목의 갯수만큼 반복을 돌린다.
				System.out.printf(" %s : %s\n", field_name[j], field[j]);	//항목의 갯수만큼 각 항목의 내용들을 출력한다.
			}
			System.out.println("*******************************");
